...
// Move API_SOURCES outside the component to avoid re-creation and fix useEffect dependency warning
const API_SOURCES = [
  {
    url: "/api/getcoinstop",
    label: "Top Gainers",
  ...
  useEffect(() => {
    if (suggestedTokens.length === 0) {
      Promise.all(
        API_SOURCES.map(async (src) => {
          try {
            ...
            if (Array.isArray(data.coins) && data.coins.length > 0) {
              ...
              return data.coins.slice(0, src.count).map((coin: Memecoin) => ({
                ...coin,
        ...
        const flat = results.flat().filter(Boolean) as (Memecoin & {
          sourceLabel: string;
          sourceColor: string;
        })[];
        const seen = new Set();
